...
def SaveTracks(simTrackFile, tracks, falarms = []) :
    ...
    for (index, track) in enumerate(tracks) :
        ...
        for centroid in track :
            dataFile.write("%(types)s %(xLocs).10f %(yLocs).10f 0.0 0.0 0.0 0 %(t)f CONSTANT VELOCITY %(cornerIDs)d\n" % 
                            centroid)

    for false_alarm in falarms :
        dataFile.write("%(xLocs).10f %(yLocs).10f %(t)f %(cornerIDs)d\n" % false_alarm[0])
...
def ReadTracks(fileName) :
    ...
    for line in open(fileName) :
        ...
        if contourCnt > 0 and centroidCnt < trackLen :
            ...
            tracks[-1]['t'][centroidCnt] = float(tempList[7])
        ...
        if len(falseAlarms) < falseAlarmCnt :
            ...
            falseAlarms.append(numpy.array([(float(tempList[0]), float(tempList[1]), int(tempList[3]), float(tempList[2]), 'F')],
...
def SaveCorners(inputDataFile, corner_filestem, volume_data, path='.', startFrame=1) :
    """
    ...
    for (frameNo, aVol) in enumerate(volume_data) :
        ...
        # Writing out the number of storm cells recorded to the inputDataFile
        dataFile.write(str(len(aVol['stormCells'])) + '\n')

    dataFile.close()

def ReadCorners(inputDataFile, path='.', times=None) :
    """
    Read corner files.
    ...
    if times is None : times = numpy.arange(startFrame, frameCnt + startFrame)

    volume_data = [{'volTime': aTime,
                    ...
                    'stormCells': numpy.atleast_1d(numpy.loadtxt("%s.%d" % (path+os.sep+corner_filestem,
                                                                            index + startFrame),
					           dtype=TrackUtils.corner_dtype,
                                                   usecols=(0, 1, 27)))}
                    for index, aTime in enumerate(times)]
